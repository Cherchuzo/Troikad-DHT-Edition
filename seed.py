...
from kad import DHT
from supportModules.transferHelpers import TroikadFileHandler, ConnectionHelper
from supportModules.TCPTools import TCPFirewall
from supportModules.hashing import Hashing
...
class Finestra(tk.Frame):
            ...
            while self.buddy:
                if self.searchKey != None:
                    ...
                    if msg != None:
                        ...
                        ####Errore probabile qui####
                        #self.nomiFile[0].config(state = "normal", text = jsonReply["filename"], peer = (jsonReply["ip"], jsonReply["port"]))
    ...
    def HashService(self, files, filepath, publicIp):
        ...
        for singleFile in chiaviFile:
            self.dht[singleFile[0]] = [json.dumps({"ip":publicIp, "port":self.PORT, "filename":singleFile[1], "metadata":[os.stat(filepath+singleFile[1]).st_size]}, ensure_ascii=False)]
            time.sleep(1)
    ...
    def FileRequest(self):
        while True:
            if self.fileToDownload != None:
                ...
                download_window.title("Download di "+self.fileToDownload[0]+"")
                tmpLabel = tk.Label(download_window, text = "Contattando peer...")
                tmpLabel.pack()

                result, connection = TroikadFileHandler.request(self.fileToDownload[0], (self.fileToDownload[1], self.fileToDownload[2]))
                if result:
                    time.sleep(2)
                    tmpLabel.destroy()
                    download_window.title("Download di "+self.fileToDownload[0]+" in corso...")
                    TroikadFileHandler.download(self.fileToDownload[0], connection, download_window)
                else:
