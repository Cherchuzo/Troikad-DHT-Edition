...
class Finestra(tk.Frame):
    ...
    def HashService(self, files, filepath, publicIp):
        ...
        for singleFile in chiaviFile:
            self.dht[singleFile[0]] = [json.dumps({"ip":publicIp, "port":2447, "filename":singleFile[1], "metadata":[os.stat(filepath+singleFile[1]).st_size]}, ensure_ascii=False)]
            time.sleep(1)
    ...
    def TcpServer(self):
        ...
        if globalFlag:
            ...
            if len(files) != 0:
                hashFiles = Thread(target=self.HashService, args = (files,filepath,publicIp))
                hashFiles.start()
